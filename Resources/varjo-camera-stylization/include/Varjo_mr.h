 ...
 *
 * This call needs to succeed before any calls to features behind the lock.
 *
 * #varjo_Unlock should be called after the feature is not needed anymore.
 *
 * Each lock can be locked by one application at a time. Locking fails if
 * another client has acquired the lock already.
 *
 ...
 *
 * Note: After unlocking the lock other applications are free to acquire the lock at any time.
 *
 * Possible errors:
 *     #varjo_Error_OperationFailed, #varjo_Error_NotAvailable, #varjo_Error_CapabilityNotAvailable,
...
/**
 * Start video see through rendering on the HMD displays.
 *
...
/**
 * Start depth estimation for the video see through image. This enables occlusion between VR and MR content
 ...
 *        the video see through camera position. Given value will be clamped to the accepted range.
 */
VARJO_API void varjo_MRSetVRViewOffset(struct varjo_Session* session, double percentage);

/**
 * Start chroma keying for the video see through image. This enables occlusion between VR and MR content
 ...
 *                        artifacts on the video see through image. The correct setting depends on the
