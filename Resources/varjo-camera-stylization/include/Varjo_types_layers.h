...
static const varjo_LayerFlags varjo_LayerFlag_ChromaKeyMasking = 0x20;     //!< Layer is masked by video see through chroma key if flag is given.
...
struct varjo_SubmitInfoLayers {
     ...
     * in #varjo_SubmitInfo when calling #varjo_BeginFrame.
     */
    int64_t frameNumber;

    /**
     * The submission flags, see #varjo_SubmitFlags for a list of valid flags.
     * @deprecated blend mode has to be specified per layer, texture submissions will be async by default.
     */
    VARJORUNTIME_DEPRECATED varjo_SubmitFlags flags;
