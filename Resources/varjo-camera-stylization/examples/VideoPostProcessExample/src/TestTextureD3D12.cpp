    ...
    : TestTexture(testType, varjoFormat, size)
{
    ...
    // Permanently map upload buffer
    {
        ...
        D3D12_SUBRESOURCE_FOOTPRINT m_subresFootprint = {};
        m_subresFootprint.Format = dxgiFormat;
        m_subresFootprint.Width = m_size.x;
        m_subresFootprint.Height = m_size.y;
        m_subresFootprint.Depth = 1;

        if (m_varjoFormat == varjo_TextureFormat_R32_FLOAT) {
            m_subresFootprint.RowPitch = static_cast<UINT>(align(m_size.x * numChannels * sizeof(float), D3D12_TEXTURE_DATA_PITCH_ALIGNMENT));
        } else if (m_varjoFormat == varjo_TextureFormat_R32_UINT) {
            m_subresFootprint.RowPitch = static_cast<UINT>(align(m_size.x * numChannels * sizeof(uint32_t), D3D12_TEXTURE_DATA_PITCH_ALIGNMENT));
        } else {
            m_subresFootprint.RowPitch = static_cast<UINT>(align(m_size.x * numChannels * sizeof(uint8_t), D3D12_TEXTURE_DATA_PITCH_ALIGNMENT));
        }

        hr = suballocateFromBuffer(m_pDataCur, m_pDataEnd, m_subresFootprint.Height * m_subresFootprint.RowPitch, D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT);
        CHECK_HRESULT(hr);

        m_uploadFootprint = {0};
        m_uploadFootprint.Offset = static_cast<UINT64>(m_pDataCur - m_pDataBegin);
        m_uploadFootprint.Footprint = m_subresFootprint;
    ...
    if (d3d12Texture) {
        if (useGPU && m_gpuSupported) {
            try {
                ...
                LOGE("Generating texture on GPU failed.");
            ...
            try {
                ...
                LOGE("Generating texture on CPU failed.");
        ...
        LOGE("Getting D3D12 texture failed.");
    }
}
