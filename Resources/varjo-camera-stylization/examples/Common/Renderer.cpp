...
{
Renderer::Mesh::Mesh(const std::vector<float>& vertexData, const std::vector<unsigned short>& indexData, Renderer::PrimitiveTopology topology)
...
std::unique_ptr<Renderer::Texture> Renderer::loadTextureFromFile(const std::string& filename)
{
    ...
    if (file.good()) {
        ...
        if (file.read(data.data(), size)) {
            ...
            LOGE("Loading texture file failed: %s", filename.c_str());
        }
    } else {
        LOGE("Opening texture file failed: %s", filename.c_str());
    }
