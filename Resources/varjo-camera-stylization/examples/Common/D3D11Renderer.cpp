...
void createBuffer(ID3D11Device* device, const void* bufdata, UINT datasize, UINT bindFlags, ID3D11Buffer** buf)
{
    CD3D11_BUFFER_DESC vDesc(datasize, bindFlags);
...
// Initialize geometry bufferes
void initGeometry(ID3D11Device* device, const std::vector<float>& vertexData, int vertexStride, const std::vector<unsigned short>& indexData,
    Renderer::PrimitiveTopology topology, D3D11Renderer::Geometry& geometry)
{
    ...
    createBuffer(device, vertexData.data(), static_cast<int>(sizeof(float) * vertexData.size()), D3D11_BIND_VERTEX_BUFFER, &geometry.vertexBuffer);
    createBuffer(device, indexData.data(), static_cast<int>(sizeof(unsigned short) * indexData.size()), D3D11_BIND_INDEX_BUFFER, &geometry.indexBuffer);
}
...
D3D11Renderer::Shader createShader(ID3D11Device* device, const D3D11Renderer::Shader::InitParams& createData)
{
    ...
    createBuffer(device, nullptr, createData.vsConstantBufferSize, D3D11_BIND_CONSTANT_BUFFER, &shader.vsConstantBuffer);

    // Create buffer for pixel shader constants
    createBuffer(device, nullptr, createData.psConstantBufferSize, D3D11_BIND_CONSTANT_BUFFER, &shader.psConstantBuffer);
...
void drawGeometry(ID3D11DeviceContext* context, const D3D11Renderer::Geometry& geometry)
{
    ...
    context->IASetIndexBuffer(geometry.indexBuffer.Get(), DXGI_FORMAT_R16_UINT, 0);
...
D3D11Renderer::RenderTarget::RenderTarget(ID3D11Device* device, int32_t width, int32_t height, ID3D11Texture2D* colorTexture, ID3D11Texture2D* depthTexture)
...
D3D11Renderer::Mesh::Mesh(ID3D11Device* device, const std::vector<float>& vertexData, int vertexStride, const std::vector<unsigned short>& indexData,
    Renderer::PrimitiveTopology topology)
    : Renderer::Mesh(vertexData, indexData, topology)
{
    initGeometry(device, m_vertices, vertexStride, m_indices, m_topology, m_geometry);
}
    ...
    : m_exampleShaders(*this)
{
...
#endif
    if (adapter == nullptr) {
        LOGW("Could not get DXGI adapter.");
    }
    ...
    if (m_device == nullptr) {
        LOGE("Could not create D3D11 device.");
    }

    if (m_context == nullptr) {
        LOGE("Could not create D3D11 device context.");
    }
...
std::unique_ptr<Renderer::Mesh> D3D11Renderer::createMesh(
    const std::vector<float>& vertexData, int vertexStride, const std::vector<unsigned short>& indexData, PrimitiveTopology topology)
{
...
void D3D11Renderer::bindRenderTarget(Renderer::RenderTarget& target)
{
    auto& d3d11Target = reinterpret_cast<D3D11Renderer::RenderTarget&>(target);

    auto rtv = d3d11Target.getRenderTargetView();
    auto dsv = d3d11Target.getDepthStencilView();
...
void D3D11Renderer::clear(
    Renderer::RenderTarget& target, const glm::vec4& colorValue, bool clearColor, bool clearDepth, bool clearStencil, float depthValue, uint8_t stencilValue)
{
